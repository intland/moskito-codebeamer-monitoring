...
var chartEngineIniter = {
    ...
    D3: function(params) {
		...
		var chartData = [],
            chartRows = params.data || [];
        chartRows = chartRows.map(
			function (d) {
				var tmp = new Date(d[0]);
				d[0] = new Date().setTime(tmp.getTime() - tmp.getTimezoneOffset()*60*1000);
				return d;
			}
		);

        for (var i = 0; i < params.names.length; i++) {
			chartData.push({"name" : params.names[i], "data" : []});
		}
		for (var i = 0; i < params.data.length; ++i) {
			for (var j = 1; j <= chartData.length; ++j) {
				chartData[j - 1].data.push([params.data[i][0], params.data[i][j]]);
			}
		}
        var chartInfo = {
            container: params.container,
        ...
        var chart = Highcharts.stockChart(chartInfo.container, {
				...
				formatter: function () {
					...
					$.each(this.points, function (i, point) {
						...
						if (formatters.hasOwnProperty(this.series.name)) {
							...
							if (formatters[this.series.name].hasOwnProperty("SUFFIX")) {
								suffix = formatters[this.series.name].SUFFIX;
							}
							if (formatters[this.series.name].hasOwnProperty("DECIMAL_PLACES")) {
						...
						s += '</b>';
						s += '<br/>';
						if (additionalInfo.hasOwnProperty(this.series.name)) {
							var accumulators = additionalInfo[this.series.name];
		...
		// handle dispatch
		var dispatch = {
			resizeLineCharts : function() {},
			refreshLineCharts : function() {},
			refreshGauge : function() {}
		};//d3.dispatch("resizeLineCharts", "refreshLineCharts", "refreshGauge");
